...
- (void)needsDisplay{
    ...
    _titleLabel.center = CGPointMake(CGRectGetWidth(_alertView.frame)/2, CGRectGetHeight(_titleLabel.frame)/2+25);
    
    CGFloat height = CGRectGetMaxY(_titleLabel.frame);
    
    if (messageText) {
        // 4.messageLabel
        _messageLabel = [[UITextView alloc] initWithFrame:CGRectMake(10, CGRectGetMaxY(_titleLabel.frame)+5, _alertView.frame.size.width-20, 35)];
